...
function Navbar({ mobileMode, isOpen: propIsOpen, onClose }: NavbarProps) {
  const [internalIsOpen, setInternalIsOpen] = useState(false);
  const [active, setActive] = useState("Dashboard");

  const isOpen = propIsOpen !== undefined ? propIsOpen : internalIsOpen;
  const setIsOpen = propIsOpen !== undefined ? 
    (newState: boolean) => {
      if (newState === false && onClose) onClose();
      if (propIsOpen === undefined) setInternalIsOpen(newState);
    } : 
    setInternalIsOpen;
